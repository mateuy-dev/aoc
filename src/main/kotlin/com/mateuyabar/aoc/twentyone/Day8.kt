...
val text8="""be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
...
val text8f="""bgafcde gfcd agc ebdgac adfceb bafeg efgca cgdfae cg ecadf | fabgced gc agc cdfg
...
fun similitudes(text1: String, text2: String)= text1.count { it in text2 }


fun main() {
    val input = text8f.lines().map { it.split("|").map { it.split(" ") } }
    val result = input.sumBy { line ->
        val values = MutableList(10){ "" }
        values[1] = line[0].first { it.count()==2 }
        values[7] = line[0].first { it.count()==3 }
        values[4] = line[0].first { it.count()==4 }
        values[8] = line[0].first { it.count()==7 }
        val fiveLines = line[0].filter {it.count()==5}.toMutableList()
        values[2] = fiveLines.first { similitudes(it, values[4]) ==2}
        fiveLines.remove(values[2])
        values[3] = fiveLines.first { similitudes(it, values[7]) ==3}
        fiveLines.remove(values[3])
        values[5] = fiveLines.first()
        val sixLines = line[0].filter {it.count()==6}.toMutableList()
        values[6] = sixLines.first { similitudes(it, values[1]) ==1}
        sixLines.remove(values[6])
        values[9] = sixLines.first { similitudes(it, values[3]) ==5}
        sixLines.remove(values[9])
        values[0] = sixLines.first()

        val valuesSorted = values.map{it.toList().sorted()}
        val reslut = line[1].filter { it!="" }.map{it.toList().sorted()}
        calculateChar(reslut[0], valuesSorted) *1000+ calculateChar(reslut[1], valuesSorted) *100+ calculateChar(reslut[2], valuesSorted) *10+ calculateChar(reslut[3], valuesSorted)
    }

    println(result)
}

fun calculateChar(s: List<Char>, values: List<List<Char>>): Int {
...
fun isSimple(it: String) = it.length in listOf(2, 3, 4, 7)
